...
const FolderMidtermRecordWorst: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      setUploadedFile(midtermRecords.worst || null);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      midtermRecords.worst = fileRecord;
      ...
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-8 flex justify-between items-center">
            ...
            <button
              onClick={() => navigate(`${basePath}/folder/${idForNav}/final/question-paper${isReviewContext ? '?review=1' : ''}`)}
          ...
          {isCoordinatorReview && submittedStatuses.has((status || '').toUpperCase()) && isReviewContext && (
            <div className="mt-6">
              <CoordinatorFeedbackBox folderId={id} section="MIDTERM_RECORDS_WORST" />
            </div>
          )}

          {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && (
            <div className="mt-6">
              <AuditMemberFeedbackBox folderId={id} section="MIDTERM_RECORDS_WORST" />
            </div>
          )}
        </div>
      </div>
...
export default FolderMidtermRecordWorst;
