...
const FolderMidtermModelSolution: React.FC = () => {
  ...
  const [solutionData, setSolutionData] = useState<MidtermSolution>({
    semester: '',
  ...
  useEffect(() => {
    ...
    courseFoldersAPI.getBasic(id)
      .then((res) => {
        ...
        // Load questions from the saved midterm question paper
        const midtermPaper = res.data.outline_content?.midtermPaper;
        const questionsFromPaper = midtermPaper?.questions || [];

        const midtermSolution = res.data.outline_content?.midtermSolution;
        if (midtermSolution) {
          const saved = midtermSolution;
        ...
        const pdfUrl = res.data.outline_content?.midtermSolution?.model_solution_pdf || null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (file) {
        ...
        formData.append('title', 'Midterm Model Solution');
      ...
      // Update any navigation links to use idForNav (if we navigate to other pages here)
  ...
  return (
    <DashboardLayout userRole={userRole}>
      <div className="p-4 md:p-6">
        ...
        <h2 className="text-base md:text-lg font-semibold text-gray-800 mb-4">Midterm Model Solution</h2>
        ...
        <div className="bg-white border border-gray-300 rounded-md shadow-sm p-6 md:p-8 max-w-4xl mx-auto">
          ...
          <div className="text-center mb-6">
            ...
            <div className="text-sm font-normal text-gray-700">Mid Term Exam</div>
          </div>
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        value={answer.clo}
                        onChange={(e) => handleUpdateAnswer(answer.id, 'clo', e.target.value)}
                        disabled={readOnly}
          ...
          <div className="mt-6 flex justify-center gap-3">
            ...
            <button
              onClick={() => navigate(`${basePath}/folder/${idForNav}/midterm/records/best${isReviewContext ? '?review=1' : ''}`)}
        ...
        {isCoordinatorReview && submittedStatuses.has((status || '').toUpperCase()) && isReviewContext && (
          <CoordinatorFeedbackBox folderId={id} section="MIDTERM_MODEL_SOLUTION" />
        )}

        {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && (
          <AuditMemberFeedbackBox folderId={id} section="MIDTERM_MODEL_SOLUTION" />
        )}
      </div>
    </DashboardLayout>
  );
...
export default FolderMidtermModelSolution;
