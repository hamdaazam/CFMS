...
const FolderMidtermRecordAverage: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      setUploadedFile(midtermRecords.average || null);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      midtermRecords.average = fileRecord;
    ...
    <DashboardLayout userRole={userRole}>
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Average Student Record</h1>
          ...
          {isCoordinatorReview && submittedStatuses.has((data?.status || '').toUpperCase()) && isReviewContext && (
            <div className="mt-6">
              <CoordinatorFeedbackBox folderId={id} section="MIDTERM_RECORDS_AVERAGE" />
            </div>
          )}

          {isAuditMemberReview && submittedStatuses.has((data?.status || '').toUpperCase()) && (
            <div className="mt-6">
              <AuditMemberFeedbackBox folderId={id} section="MIDTERM_RECORDS_AVERAGE" />
            </div>
          )}
        </div>
      </div>
...
export default FolderMidtermRecordAverage;
