...
const FolderAssignmentRecordAverage: React.FC = () => {
    ...
    <DashboardLayout userRole={userRole}>
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">{assignmentName} - Average Student Record</h1>
            ...
            {uploadedFile ? (
              ...
              <div>
                ...
                <h3 className="text-xl font-semibold text-gray-800 mb-2 text-center">
                  {isDragging ? 'Drop your file here' : 'Upload Average Student Records'}
                </h3>
          ...
          <div className="mt-8 flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
            ...
            <div className="flex gap-4 justify-end">
              <button
                onClick={() => navigate(`${basePath}/folder/${id}/assignments/${assignmentId}/records/best${isReviewContext ? '?review=1' : ''}`)}
                className="px-6 py-3 bg-gray-600 text-white rounded-full hover:bg-gray-700 font-medium transition-colors"
              >
                Previous: Best Record
              </button>
              <button
                onClick={() => navigate(`${basePath}/folder/${id}/assignments/${assignmentId}/records/worst${isReviewContext ? '?review=1' : ''}`)}
      ...
      {isReviewContext && submittedStatuses.has((status || '').toUpperCase()) && <CoordinatorFeedbackBox folderId={id!} section={`ASSIGNMENT_${assignmentId}_AVERAGE`} />}

      {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && <AuditMemberFeedbackBox folderId={id!} section={`${assignmentName} - Average Record`} />}
    </DashboardLayout>
  );
};

export default FolderAssignmentRecordAverage;
