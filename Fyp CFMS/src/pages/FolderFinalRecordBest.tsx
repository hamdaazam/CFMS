...
const FolderFinalRecordBest: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      setUploadedFile(finalRecords.best || null);
    } catch (err) {
      ...
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-8 flex justify-between items-center">
            <button
              onClick={() => navigate(`${basePath}/folder/${id}/final/model-solution${isReviewContext ? '?review=1' : ''}`)}
          ...
          {isCoordinatorReview && submittedStatuses.has((status || '').toUpperCase()) && isReviewContext && (
            <div className="mt-6">
              <CoordinatorFeedbackBox folderId={id} section="FINAL_RECORDS_BEST" />
            </div>
          )}

          {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && (
            <div className="mt-6">
              <AuditMemberFeedbackBox folderId={id} section="FINAL_RECORDS_BEST" />
            </div>
          )}
        </div>
      </div>
...
export default FolderFinalRecordBest;
