...
const FolderQuizRecordBest: React.FC = () => {
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">{quizName} - Best Student Record</h1>
          ...
          <div className="mt-8 flex justify-between items-center">
            <button
              onClick={() => navigate(`${basePath}/folder/${id}/quizzes/${quizId}/records${isReviewContext ? '?review=1' : ''}`)}
      ...
      {isReviewContext && submittedStatuses.has((status || '').toUpperCase()) && <CoordinatorFeedbackBox folderId={id!} section={`QUIZ_${quizId}_BEST`} />}

      {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && <AuditMemberFeedbackBox folderId={id!} section={`${quizName} - Best Record`} />}
    </DashboardLayout>
  );
};

export default FolderQuizRecordBest;
