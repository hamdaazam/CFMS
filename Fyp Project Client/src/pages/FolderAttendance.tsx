...
const FolderAttendance: React.FC = () => {
  ...
  const [uploadedFile, setUploadedFile] = useState<AttendanceFile | null>(null);
  ...
  // Define statuses that should force read-only (submitted/under-review/approved/etc.)
  ...
  useEffect(() => {
    ...
    courseFoldersAPI.getBasic(id)
      .then((res) => {
        ...
        setReadOnly(!canEdit);
        if (res.data.outline_content?.attendanceFile) {
          setUploadedFile(res.data.outline_content.attendanceFile);
        }
      })
      .catch(() => {
        if (mounted) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (files.length > 0 && files[0].type === 'application/pdf') {
      if (!readOnly) setSelectedFile(files[0]);
    } else {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const outlineContent = {
        ...existingContent,
        attendanceFile: fileData,
      };

      await courseFoldersAPI.saveOutline(id, { outline_content: outlineContent });
      setUploadedFile(fileData);
      setSelectedFile(null);
      alert('Attendance record uploaded successfully!');
    } catch (error) {
      console.error('Error uploading attendance:', error);
      alert('Failed to upload attendance record');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <DashboardLayout userRole={userRole}>
      <div className="p-4 md:p-6">
        ...
        <h2 className="text-base md:text-lg font-semibold text-gray-800 mb-6">{isCoordinatorReview ? 'Attendance Record' : 'Upload Attendance Record'}</h2>
          ...
          >
            <div className="text-center">
              {uploadedFile ? (
                ...
                <div className="mb-4">
                  ...
                  <p className="text-base font-medium text-gray-700 mb-1">
                    Drag & Drop your attendance PDF here
                  </p>
              ...
              <div className="flex items-center justify-center gap-3">
                <label className={`cursor-pointer ${readOnly ? 'opacity-50 pointer-events-none' : ''}`}>
                  <input
                    type="file"
        ...
        <div className="mt-8 flex justify-between items-center">
          ...
          <button
            onClick={() => navigate(`${basePath}/folder/${id}/lecture-notes${isReviewContext ? '?review=1' : ''}`)}
        ...
        {isReviewContext && submittedStatuses.has((data?.status || '').toUpperCase()) && (
          <CoordinatorFeedbackBox folderId={id} section="ATTENDANCE" />
        )}

        {isAuditMemberReview && submittedStatuses.has((data?.status || '').toUpperCase()) && (
          <AuditMemberFeedbackBox folderId={id} section="ATTENDANCE" />
        )}
      </div>
    </DashboardLayout>
  );
...
export default FolderAttendance;
