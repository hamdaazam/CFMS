...
const FolderAssignmentRecordWorst: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      const assignmentsList = outlineContent.assignments || [];
      setAssignments(assignmentsList);
      const assignmentIdx = assignmentsList.findIndex((a: any) => a.id === assignmentId);
      setCurrentIndex(assignmentIdx);
      const currentAssignment = assignmentsList[assignmentIdx];
      if (currentAssignment) {
        setAssignmentName(currentAssignment.name);
      }
      
      // Load quizzes for potential navigation
      const quizzesList = outlineContent.quizzes || [];
      setQuizzes(quizzesList);
      
      const assignmentRecords = outlineContent.assignmentRecords || {};
      const records = assignmentRecords[assignmentId] || {};

      setUploadedFile(records.worst || null);
    } catch (err) {
    ...
    <DashboardLayout userRole={userRole}>
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">{assignmentName} - Worst Student Record</h1>
            <p className="text-gray-600">Upload PDF record for worst performing student</p>
          </div>
              ...
              <div className="text-center">
                ...
                <div className="flex flex-col gap-3">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-orange-600 text-white rounded-full hover:bg-orange-700 font-medium transition-colors"
                  >
                    <Download size={18} />
                    Download File
                  </button>
          ...
          <div className="mt-8 flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
            ...
            <div className="flex gap-4 justify-end">
              {currentIndex >= 0 && currentIndex < assignments.length - 1 ? (
                <button
                  onClick={() => navigate(`${basePath}/folder/${id}/assignments/${assignments[currentIndex + 1].id}/question-paper${isReviewContext ? '?review=1' : ''}`)}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 font-medium transition-colors"
                >
                  Next: Assignment {currentIndex + 2}
                </button>
              ) : quizzes.length > 0 ? (
                <button
                  onClick={() => navigate(`${basePath}/folder/${id}/quizzes/${quizzes[0].id}/question-paper${isReviewContext ? '?review=1' : ''}`)}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 font-medium transition-colors"
                >
                  Next: Quiz 1
                </button>
                ...
                <button
                  onClick={() => navigate(`${basePath}/folder/${id}/midterm/question-paper${isReviewContext ? '?review=1' : ''}`)}
                  className="px-6 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 font-medium transition-colors"
                >
                  Next: Midterm
                </button>
      ...
      {isReviewContext && submittedStatuses.has((status || '').toUpperCase()) && <CoordinatorFeedbackBox folderId={id!} section={`ASSIGNMENT_${assignmentId}_WORST`} />}

      {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && <AuditMemberFeedbackBox folderId={id!} section={`${assignmentName} - Worst Record`} />}
    </DashboardLayout>
  );
};

export default FolderAssignmentRecordWorst;
