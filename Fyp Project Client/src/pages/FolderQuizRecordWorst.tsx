...
const FolderQuizRecordWorst: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      const quizIdx = quizzesList.findIndex((q: any) => q.id === quizId);
      setCurrentIndex(quizIdx);
      const currentQuiz = quizzesList[quizIdx];
      if (currentQuiz) {
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">{quizName} - Worst Student Record</h1>
          ...
          <div className="mt-8 flex justify-between items-center">
            <button
              onClick={() => navigate(`${basePath}/folder/${id}/quizzes/${quizId}/records/average${isReviewContext ? '?review=1' : ''}`)}
            ...
            {currentIndex >= 0 && currentIndex < quizzes.length - 1 ? (
              <button
                onClick={() => navigate(`${basePath}/folder/${id}/quizzes/${quizzes[currentIndex + 1].id}/question-paper${isReviewContext ? '?review=1' : ''}`)}
                className="px-6 py-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 font-medium transition-colors"
              >
                Next: Quiz {currentIndex + 2}
              </button>
            ) : (
              <button
      ...
      {isReviewContext && submittedStatuses.has((status || '').toUpperCase()) && <CoordinatorFeedbackBox folderId={id!} section={`QUIZ_${quizId}_WORST`} />}

      {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && <AuditMemberFeedbackBox folderId={id!} section={`${quizName} - Worst Record`} />}
    </DashboardLayout>
  );
};

export default FolderQuizRecordWorst;
