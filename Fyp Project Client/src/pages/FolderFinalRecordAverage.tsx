...
const FolderFinalRecordAverage: React.FC = () => {
  ...
  const loadRecords = async () => {
    ...
    try {
      ...
      setUploadedFile(finalRecords.average || null);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      finalRecords.average = fileRecord;
      ...
      <div className="min-h-screen bg-[#F4F7FE] p-8">
        <div className="max-w-3xl mx-auto">
          ...
          {isCoordinatorReview && submittedStatuses.has((status || '').toUpperCase()) && isReviewContext && (
            <div className="mt-6">
              <CoordinatorFeedbackBox folderId={id} section="FINAL_RECORDS_AVERAGE" />
            </div>
          )}

          {isAuditMemberReview && submittedStatuses.has((status || '').toUpperCase()) && (
            <div className="mt-6">
              <AuditMemberFeedbackBox folderId={id} section="FINAL_RECORDS_AVERAGE" />
            </div>
          )}
        </div>
      </div>
...
export default FolderFinalRecordAverage;
